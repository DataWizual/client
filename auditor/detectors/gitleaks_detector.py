...
_SECRET_VALUE_RE = _re.compile(
    r"(?i)(password|secret|token|key|api_key|auth)\s*[=:]\s*\S+",
)


def _redact_secret_line(line: str) -> str:
    """Replaces secret values in raw Gitleaks Line field with [REDACTED]."""
    if not line:
        return line
    return _SECRET_VALUE_RE.sub(
        lambda m: m.group(0).split("=")[0] + "=[REDACTED]", line
    )
            ...
            for leak in findings_data:
                try:
                    ...
                    fnd = Finding(
                        ...
                        cvss_score=cvss,
                        meta={
                            "offending_line": _redact_secret_line(leak.get("Line", "")),
